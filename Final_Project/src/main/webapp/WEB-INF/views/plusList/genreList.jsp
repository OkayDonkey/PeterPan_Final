		...
		<div class="table_cont">
		 ...
		 <div class="pagingout">
		   <c:if test="${paging.page > paging.block }">
		      <a href="genreList.go?page=1&bookCategory=${category }&bookGenre=${genre }">[처음으로]</a>
		      <a href="genreList.go?page=${paging.startBlock - 1 }&bookCategory=${category }&bookGenre=${genre }">◀</a>
		   </c:if>
		   ...
		   <c:forEach begin="${paging.startBlock }"
		      ...
		      <c:if test="${i == paging.page }">
		         <b><a href="genreList.go?page=${i }&bookCategory=${category }&bookGenre=${genre }">[${i }]</a></b>
		      </c:if>
		      
		      <c:if test="${i != paging.page }">
		         <a href="genreList.go?page=${i }&bookCategory=${category }&bookGenre=${genre }">[${i }]</a>
		      </c:if>
		      
		   </c:forEach>
		   
		   <c:if test="${paging.endBlock < paging.allPage }">
		      <a href="genreList.go?page=${paging.endBlock + 1 }&bookCategory=${category }&bookGenre=${genre }">▶</a>
		      <a href="genreList.go?page=${paging.allPage }&bookCategory=${category }&bookGenre=${genre }">[마지막으로]</a>
		   </c:if>
		 </div>
	</div>
