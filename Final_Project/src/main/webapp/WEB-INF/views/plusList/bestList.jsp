		...
		<div class="table_cont">
	 	 ...
	 	 <div class="pagingout">
			   <c:if test="${paging.page > paging.block }">
			      <a href="bestListbook.go?page=1">[처음으로]</a>
			      <a href="bestListbook.go?page=${paging.startBlock - 1 }">◀</a>
			   </c:if>
			   ...
			   <c:forEach begin="${paging.startBlock }"
			      ...
			      <c:if test="${i == paging.page }">
			         <b><a href="bestListbook.go?page=${i }">[${i }]</a></b>
			      </c:if>
			      
			      <c:if test="${i != paging.page }">
			         <a href="bestListbook.go?page=${i }">[${i }]</a>
			      </c:if>
			   </c:forEach>
			   
			   <c:if test="${paging.endBlock < paging.allPage }">
			      <a href="bestListbook.go?page=${paging.endBlock + 1 }">▶</a>
			      <a href="bestListbook.go?page=${paging.allPage }">[마지막으로]</a>
			   </c:if>
	   </div>
	</div>
